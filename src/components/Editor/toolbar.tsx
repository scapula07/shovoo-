...
export default function Toolbar({workflow}:any) {
 ...
 const publish=async()=>{
      setLoading(true)
      try{
        const res= await workflowApi.publishWorkflow(executionGraph,workflow?.id)
        setLoading(false)
      }catch(e){
        console.log(e)
        setLoading(false)
  ...
  return (
    <div className='w-full flex px-10 py-8 border-b justify-between'>
            ...
            <div className='flex items-center border-r-2 border-slate-700 px-4 '>
                 ...
                 {edit?
                      <div className='flex space-x-2 items-center'>
                         <button className='text-xs border py-2 px-4 rounded-sm border-purple-500 font-semibold text-purple-500' onClick={editName}>
                          {isEditing?
                             "Updating..."
                              :
                              "Update"
                         ...
                         <IoIosCloseCircle onClick={()=>setEdit(false)} className="text-xl"/>
                    
                      </div>
                      :
                   <MdEdit className='text-gray-600 font-bold text-xl' onClick={()=>setEdit(true)}/>
                 }
         ...
         <div className='flex items-center justify-end space-x-6'>
             <button className='text-sm bg-gray-200 px-4 py-2'
                  onClick={saveChanges}
              >
             {isSave?
                  "Saving..."
